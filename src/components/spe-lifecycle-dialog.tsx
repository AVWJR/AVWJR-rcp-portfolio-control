...
export function SpeLifecycleDialog({
  ...
  async function submit() {
    ...
    setError(null);
    const href =
      action === "archive" ? `/api/deals/${encodeURIComponent(code)}/archive` : `/api/archive/${encodeURIComponent(code)}/restore`;
    try {
      ...
      if (!res.ok) {
        setError(json.error || (res.status === 403 ? "Partner view cannot archive or restore deals." : "Request failed."));
        setBusy(false);
        return;
      }
      close(true);
  ...
  const confirmVerb = action === "archive" ? "Archive deal" : "Restore deal";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-950/50 px-4" role="presentation" onClick={close}>
          ...
          >
            <p className="text-[11px] uppercase tracking-[0.2em] text-gold-700">
              {action === "archive" ? "Soft-archive" : "Restore"} · two-step confirm
            </p>
            <h2 id={titleId} className="mt-1 font-display text-2xl text-navy-900">
              {action === "archive" ? `Archive ${code}` : `Restore ${code}`}
            </h2>
            {step === 1 ? (
              <>
              ...
              <>
                <p className="mt-3 text-sm text-ink-700">
                  Type <strong>{code}</strong> to confirm. Archived deals are studied on gold nav{" "}
                  <strong>Archive</strong>, not under Deals.
                </p>
