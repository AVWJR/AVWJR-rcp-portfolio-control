...
import { archiveImpactCopy, liveSpeWhere } from "@/lib/archive";
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-ink-700">
            Live property SPEs under OpCo only. Archived deals are not listed here — study and restore
            them from gold nav <strong>Archive</strong>. Add a new deal when you are onboarding a new SPE,
            not a new HoldCo or a second OpCo.
          </p>
        </div>
        {role === "principal" ? (
          <Link
          ...
          <p className="text-sm text-ink-600">Partner view — Add Deal and archive are off.</p>
        )}
      </div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              {role === "principal" ? (
                <SpeLifecycleDialog
                  ...
                  code={spe.code}
                  name={spe.name}
                  impact={archiveImpactCopy({ code: spe.code, name: spe.name })}
                  triggerLabel="Archive deal…"
