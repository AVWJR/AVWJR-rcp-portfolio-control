    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-ink-700">
          Soft-archived property SPEs for study. Books and vault stay intact. These deals are{" "}
          <strong>not</strong> under Deals and they are out of the OpCo combined roll-up. Restore is a
          Principal two-step confirm from this page only.
        </p>
        <p className="mt-2 text-sm text-ink-600">
          ...
          . Archive a live deal from the Deals row action or from that SPE’s Vault.
        </p>
      </div>
      ...
      {deals.length === 0 ? (
        <div className="border border-cream-300 bg-white px-6 py-8 shadow-ledger">
          ...
          <p className="mt-2 max-w-xl text-sm text-ink-700">
            When you archive a live SPE, it leaves Deals and lands here. Nothing is wiped.
          </p>
        </div>
      ) : (
          ...
          {deals.map((spe) => (
            <article key={spe.code} className="border border-cream-300 bg-white px-5 py-4 shadow-ledger">
              ...
              <p className="mt-1 text-sm text-ink-700">
                ...
                {spe.archivedAt ? ` · archived ${spe.archivedAt.toISOString().slice(0, 10)}` : ""}
              </p>
