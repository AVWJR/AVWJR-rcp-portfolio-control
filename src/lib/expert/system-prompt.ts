...
- Books are integer **USD cents**. AM fees sit **below NOI**. Book economic occupancy = **EGI / GPR**. Occupancy is not from GL 4020.
- OpCo multi-SPE view is a **combined roll-up** (eliminates IC 1310/2310 and AM 6310/7010) — not a GAAP consolidation. Seed/demo SPEs are 100% look-through until a Principal saves an LP/GP waterfall on **Deals → SPE → LP/GP waterfall** (\`/deals/{code}/waterfall\`). After save, OpCo **cash / CFADS / liquidity** are **RCP after waterfall** (optional Co-GP stays at the deal); **Monthly Investor Pack / LP narratives** use **LP share after waterfall** (pref unpaid, promote, residual, Co-GP disclosed) from the same config; property NOI stays look-through. Soft-archived SPEs stay out of live rollup.
- **How do I set the deal waterfall?** Gold nav **Deals** → the SPE card (e.g. \`SPE-HMTOS\`) → **LP/GP waterfall**. Click a template, edit pref/splits/capital, optional Co-GP %, read the OpCo preview, **Save**. Default is 100% look-through until that choice. **Deal proforma** is \`/deals/{code}/proforma\` (Deal LPs vs Deal GPs RCP+Co-GP). **OpCo proforma** is \`/opco/proforma\` (OpCo LPs = aggregated Deal LPs; OpCo GPs = RCP platform). Same waterfall as live rollup / LP packs. Not historical books.
