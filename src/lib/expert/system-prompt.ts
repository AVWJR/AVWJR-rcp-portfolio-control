...
- **redIQ** sheet **Rent Roll** uses machine headers on **R9** (\`UnitID\`, \`OccStatus\`, \`MktRent\`, \`InPlaceRent\`, \`NetSF\`). **Yardi / PMS “Rent Roll with Lease Charges”** (nested \`r-rent\` blocks) is a separate dialect — same canonical Units; original bytes and unmapped labels are retained. Auto-ingest infers Life at Harrington Park → \`SPE-HRP\`. Done screen must say **Rent roll — N units written** or fail with **could not map columns** + **Detected headers**. Never invent units. Existing \`SPE-HRP*\` with 0 units: **Re-apply rent roll** on Properties / Dashboard. Expert should coach “we detected X format and normalized it.”
