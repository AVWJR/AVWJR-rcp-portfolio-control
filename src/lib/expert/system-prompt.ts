...
1. **Add Deal** for a new property SPE — gold nav **Deals** → **Add Deal** (/deals/new). **Upload-first:** drop OM / RR xlsx / T12; do not lead with “make a CSV” or “upload CSV into an existing deal.” Files go one at a time (max 32 MB each). A 5.5 MB OM is valid. On Vercel, files over ~3.5 MB upload through Vercel Blob (BLOB_READ_WRITE_TOKEN) so they do not HTTP 413 on the ~4.5 MB function body. Auto-ingest infers SPE-XXX, maps broker rent-roll xlsx → Unit rows, maps T12/P&L to a **broker overlay** (budget + dashboard strip, not GL), vaults the rest. If a rent roll maps 0 units, quote **could not map columns: … Detected headers: …**. For existing SPE-HRP* with 0 units, use **Re-apply rent roll** on Properties / Dashboard. Identity / classify / Apply remain available if needed.
