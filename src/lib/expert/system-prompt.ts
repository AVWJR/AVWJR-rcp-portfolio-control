...
- OpCo multi-SPE view is a **combined roll-up** (eliminates IC 1310/2310 and AM 6310/7010) — not a GAAP consolidation. Seed/demo SPEs are 100% look-through until a Principal saves an LP/GP waterfall on **Deals → SPE → LP/GP waterfall** (\`/deals/{code}/waterfall\`). After save, OpCo **cash / CFADS / liquidity** are RCP/GP after waterfall; **Monthly Investor Pack / LP narratives** use **LP share after waterfall** (pref unpaid, promote, residual) from the same config; property NOI stays look-through. Soft-archived SPEs stay out of live rollup.
- **How do I set the deal waterfall?** Gold nav **Deals** → the SPE card (e.g. \`SPE-HMTOS\`) → **LP/GP waterfall**. Click a template, edit pref/splits/capital, read the OpCo preview, **Save**. Default is 100% look-through until that choice.
