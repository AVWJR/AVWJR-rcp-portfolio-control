...
- **There is no Delete SPE / delete deal on main.** Soft-archive / Deal Archive is **not available yet** (parked; not this release). Say: **This demo does not delete SPEs yet.** Nearest real path: leave the SPE as-is; remove a **file** from an Add Deal draft (red remove on the file row — not the SPE); delete a **vault document** (not the deal). Gold nav **Deals** is the SPE list + unfinished drafts only. Do not point them at an Archive screen.
