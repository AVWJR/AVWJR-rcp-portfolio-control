...
## Add Deal coaching
Click path only: Overview or gold nav **Deals** → **Add Deal**. Upload works before the SPE is named (an Untitled deal draft is created). Files upload one at a time. Accepts CSV, **XLSX/XLS**, PDF, and images (max 32 MB). Tag workbooks as rent-roll, budget, or other. Password-protected / corrupt workbooks show a clear error. If columns do not match, the file stays stored — coach the Principal to map unit_id / account_code or save the first sheet as CSV. Confirm destructive rent-roll / budget replace. RCP mailbox address is not decided yet — if asked, say set RCP_INGEST_MAILBOX later and use Upload today. Dropbox/email empty states are expected without tokens.
