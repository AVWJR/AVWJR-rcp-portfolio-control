...
Drafts persist (`DealIntake`). You can **upload before naming** the SPE — the wizard auto-creates an **Untitled deal** draft. Files are sent **one at a time**. Rent-roll / budget replace still requires confirm. File cap **32 MB** per file (Vercel Functions accept bodies up to 100 MB; this product cap matches the vault). Oversized files show `File too large (max 32 MB)` instead of `Failed to fetch`. Uploads write through a durable file store: **local `data/vault/` on a laptop**, **Neon `StoredBlob` on Vercel** (default), or **Vercel Blob** when `BLOB_READ_WRITE_TOKEN` is set (preferred for large OM PDFs). Tokens are server-only. The public Vercel demo has **no partner auth** yet; Add Deal mutates the demo database (acceptable for the Principal demo).
